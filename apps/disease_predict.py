...
# Symptom dictionary (full from training)
...
if "recommended_doctor" not in st.session_state:
    st.session_state.recommended_doctor = None

# Streamlit App
st.title("🩺 Disease Prediction System using Symptoms")
st.markdown("Enter your symptoms to predict possible diseases.")
...
with st.form("symptom_form"):
    ...
    if submitted:
        if symptoms_input.strip() == "":
            st.warning("⚠️ Please enter valid symptoms (comma separated)")
        else:
            user_symptoms = [s.strip().lower() for s in symptoms_input.split(',')]
            predicted_disease = get_predicted_value(user_symptoms)
            desc, pre, med, diet, wrkout = helper(predicted_disease)

            recommended_doctor = doctor_mapping.get(predicted_disease, "General Physician")

            # Store in session state for saving later
            st.session_state.predicted_disease = predicted_disease
            st.session_state.selected_symptoms = user_symptoms
